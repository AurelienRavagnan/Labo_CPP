...
void ApplicGarageWindow::on_actionNewModel_triggered()
{
    ...
    if (modelName.empty() || power < 1 || engine < 0 || engine > 3 || basePrice < 0 || image.empty()) {
        this->dialogError("Erreur","Données invalides pour le nouveau modèle.");
        return;
    }

    // Création du modèle et ajout au garage
    try {
        Model m(modelName, power, static_cast<Engine>(engine), basePrice, image);
        Garage::getInstance().addModel(m);
        this->addAvailableModel(modelName, basePrice);
        cout << "Modèle ajouté avec succès : " << m.toString() << endl;
    }
    catch (const exception& e) {
        this->dialogError("Erreur lors de l’ajout du modèle", e.what());
    }
}
...
void ApplicGarageWindow::on_actionNewOption_triggered()
{
    ...
    if (code.empty() || label.empty() || price < 0)
    {
        this->dialogError("Erreur", "Données invalides pour la nouvelle option.");
        return;
    }

    try
    {
        Option opt(code, label, price);

        // Ajout dans le garage
        Garage::getInstance().addOption(opt);

        // Mise à jour interface
        this->addAvailableOption(label, price);

        cout << "Option ajoutée avec succès : " << opt.toString() << endl;
    }
    catch (const OptionException& e)
    {
        this->dialogError("Erreur - Option invalide", e.getMessage().c_str());
    }
    catch (const Exception& e)
    {
    ...
    {
        this->dialogError("Erreur inattendue", "Une erreur inconnue est survenue.");
    }
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void ApplicGarageWindow::on_pushButtonSelectModel_clicked()
{
    ...
    int indexModel = this->getIndexModelSelectionCombobox();
    cout << "Index du modèle sélectionné : " << indexModel << endl;

    if (indexModel == -1)
    {
        this->dialogError("Erreur", "Aucun modèle sélectionné.");
        return;
    }

    try
    {
        Model selectedModel = Garage::getInstance().getModel(indexModel);

        Garage::getInstance().getCurrentProject().setModel(selectedModel);

        this->setModel(
            selectedModel.getName(),
            selectedModel.getPower(),
            static_cast<int>(selectedModel.getEngine()),
            selectedModel.getBasePrice(),
            selectedModel.getImage()
        );
        

        this->setPrice(Garage::getInstance().getCurrentProject().getPrice());
        for (int i = 0;i<5;i++){
            this->setTableOption(i);
        }
        cout << "Modèle sélectionné : " << selectedModel.toString() << endl;
    }
    catch (const Exception& e)
    {
        this->dialogError("Erreur lors de la sélection du modèle", e.getMessage().c_str());
    }
}
...
void ApplicGarageWindow::on_pushButtonAddOption_clicked()
{
    ...
    int indexOption = this->getIndexOptionSelectionCombobox();
    cout << "Index de l'option sélectionnée : " << indexOption << endl;
    ...
    try
    {
        Option selectedOption = Garage::getInstance().getOption(indexOption);
        Garage::getCurrentProject().addOption(selectedOption);

        cout << "Option ajoutée : " << selectedOption.toString() << endl;

        auto list = Garage::getCurrentProject().getOptions();
        ...
        this->setPrice(Garage::getCurrentProject().getPrice());
    }
    catch (const OptionException& e)
    {
        this->dialogError("Erreur Option invalide", e.getMessage().c_str());
    ...
    {
        this->dialogError("Erreur inattendue", e.getMessage().c_str());
    }
}
...
void ApplicGarageWindow::on_pushButtonRemoveOption_clicked()
{
    ...
    int indexOption = this->getIndexOptionSelectionTable();
    cout << "Index de l'option sélectionnée dans la table : " << indexOption << endl;

    if (indexOption == -1)
    {
        this->dialogError("Erreur", "Aucune option sélectionnée dans la table.");
        return;
    }

    try {
        Option* optionToRemove = Garage::getCurrentProject()[indexOption];

        cout << "Option supprimée : " << optionToRemove->toString() << endl;
        Garage::getCurrentProject().removeOption(optionToRemove->getCode());
        
        const auto& list = Garage::getCurrentProject().getOptions();
        ...
        this->dialogError("Erreur lors de la suppression de l’option", e.what());
    }
}
...
void ApplicGarageWindow::on_pushButtonReduction_clicked()
{
    ...
    try{
        Option* opt = Garage::getCurrentProject()[indexOption];

        if(opt==nullptr)
        {
            this->dialogError("Erreur","Option non trouvée dans le projet en cours.");
            return;
        }
        cout<< "Option avant réduction : " << opt->toString() << endl;
        double newPrice = opt->getPrice()-50;
        ...
        this->setPrice(Garage::getCurrentProject().getPrice());
        cout << "Option après réduction : " << opt->toString() << endl;
    } 
    catch (const Exception& e) {
        this->dialogError("Erreur lors de la réduction de l’option", e.getMessage().c_str());
    }
}
...
void ApplicGarageWindow::on_pushButtonSaveProject_clicked()
{
    ...
    string projectName = this->getCurrentProjectName();
    cout << "Nom du projet à sauvegarder : " << projectName << endl;
    if (projectName.empty())
    {
        this->dialogError("Erreur","Le nom du projet ne peut pas être vide.");
        return;
    }
    try{
        Garage::getCurrentProject().setName(projectName);
        Garage::getCurrentProject().save(projectName);
        cout << "Projet sauvegardé avec succès : " << projectName << endl;
    }
    catch (const Exception& e) {
        this->dialogError("Erreur lors de la sauvegarde du projet", e.getMessage().c_str());
    }
}
...
void ApplicGarageWindow::on_pushButtonOpenProject_clicked()
{
    ...
    string projectName = dialogPromptText("Ouverture", "Entrez le nom du fichier :");
    cout << "Nom du projet à ouvrir : " << projectName << endl;
    ...
    try {
        ...
        Car& proj = Garage::getCurrentProject();

        this->setCurrentProjectName(proj.getName());

        this->setModel(
            proj.getModel().getName(),
            proj.getModel().getPower(),
            static_cast<int>(proj.getModel().getEngine()),
            proj.getModel().getBasePrice(),
            proj.getModel().getImage()
        );

        this->setPrice(proj.getPrice());

        // Table des options du projet
        for (int i = 0; i < 5; i++)
            this->setTableOption(i);

        for (int i = 0; i < 5; i++)
        {
            Option* optPtr = proj[i];
            if (optPtr != nullptr)
            {
                this->setTableOption(
                    i,
        ...
        this->clearComboBoxAvailableModels();
        int modelCount = 0; 
        try {
            while (true) {
                Model m = Garage::getInstance().getModel(modelCount);
                this->addAvailableModel(m.getName(), m.getBasePrice());
                modelCount++;
            }
        } catch (...) {
        ...
        this->clearComboBoxAvailableOptions();
        int optionCount = 0;
        try {
            while (true) {
                Option o = Garage::getInstance().getOption(optionCount);
                this->addAvailableOption(o.getLabel(), o.getPrice());
                optionCount++;
            }
        } catch (...) {
            // fin de liste atteinte
        ...
        cout << "Projet ouvert avec succès : " << projectName << endl;
    }
    catch (const Exception& e) {
        this->dialogError("Erreur lors de l’ouverture du projet", e.getMessage().c_str());
    }
}
...
void ApplicGarageWindow::on_pushButtonNewProject_clicked()
{
    ...
    try{
        ...
        this->setModel("", 0, 0, 0.0, "");
        this->clearTableOption();
        this->setPrice(0.0);
        cout << "Nouveau projet créé avec succès." << endl;
    }
    catch (const Exception& e) {
        this->dialogError("Erreur lors de la création du nouveau projet", e.getMessage().c_str());
    }
