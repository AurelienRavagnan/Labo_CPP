...
vector<Option> Car::getOptions() const
{
    vector<Option> list;
    ...
    {
        if (options[i] != nullptr)
            list.push_back(*options[i]);
    }
    return list;
}

void Car::save(const string& projectName)
{
    XmlFileSerializer<Car>serializer(projectName +".xml",XmlFileSerializer<Car>::WRITE,"Car");
    serializer.write(*this);
}

void Car::load(string projectName)
{
    XmlFileSerializer<Car>serializer(projectName+".xml",XmlFileSerializer<Car>::READ,"Car");
